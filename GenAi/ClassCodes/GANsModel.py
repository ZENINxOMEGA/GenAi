...
def generator(rv_dim =50):
    ...
    model.add(keras.layers.Dense(784,activation='leaky_relu'))
...
def generate_real_samples(data,n_samples = 100):
    ...
    X_train = data[ix]
    y=np.ones(shape=(X_train.shape[0],))
    return X_train,y

def genrate_rv(rv_dim,n_sample=100):
    return np.random.randn(n_sample,rv_dim)

def generate_fake_images(g_model,rv_dim,n_samples = 100):
    rv = genrate_rv(rv_dim,n_samples)
...
def save_fig(g_model,rv_dim,epoch):
    import os
    os.makedirs('GenAi/ClassCodes/GAN_output', exist_ok=True)
    n = 10
    rv = genrate_rv(rv_dim,n*n)
    f_imgs = g_model.predict(rv)
    for i in range(n*n):
        plt.subplot(n,n,1+i)
        plt.axis('off')
        plt.imshow(f_imgs[i].reshape((28,28)), interpolation='nearest',cmap='gray')
    filename = f'GenAi/ClassCodes/GAN_output/generated_plot_e{epoch}.png'
    plt.savefig(filename)
    plt.close()

def train(data,g_model,d_model,gan,rv_dim,epochs = 50, batch_size = 256):
    ...
    for e in range(epochs):
        for bn in range(nbatchs):
            ...
            x_rv = genrate_rv(rv_dim,batch_size)
            y = np.ones((batch_size,1))
            g_loss,_ = gan.train_on_batch(x_rv,y)
